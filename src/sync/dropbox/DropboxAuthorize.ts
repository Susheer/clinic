...
export class DropboxAuthorize implements Authorize {
  ...
  public authorize(): Promise<void> {
    ...
    if (this.urlHandlerIsListening && this.handleOpenURL) {
      console.log("// There is already a listener! Remove it.")
    ...
    .catch((err) =>{
        console.error("An error occurred trying to open the browser to authorize with Dropbox:", err)
      })
      .then(() => {
        console.log("open url redirct")
          ...
          this.handleOpenURL = (event: { url: string }) => {
            this._handleOpenURL(event, stateValue)
              ...
              .then(() => {
                ...
                // "Finally" block
                console.log("// Remove deep link event listener");
  ...
  public hasUserAuthorized(): Promise<boolean> {
    console.log("hasUserAuthorized invoked")
  ...
  public revokeAuthorization(): Promise<void> {
    console.log("revokeAuthorization invoked")
  ...
  private _handleOpenURL(event: { url: string }, stateValue: string): Promise<void> {
    console.log("_handleOpenURL invoked");
    ...
    console.log("Dropbox-response----->parsedQueryString",parsedQueryString)
