...
export class DropboxDatabaseSync implements DatabaseSync {
  ...
  public hasRemoteUpdate(): Promise<boolean> {
    console.log("hasRemoteUpdate is invoked");
  ...
  public hasLastUploadCompleted(): Promise<boolean> {
    console.log("hasLastUploadCompleted is invoked");
  ...
  public download(): Promise<void> {
    console.log("download is invoked");
    return AsyncStorage.getItem(DROPBOX.ACCESS_TOKEN_STORAGE_KEY)
      .then((accessToken) => {
        if (accessToken === null) {
          // Not wired up to Dropbox, so there cannot be a remote DB update
  ...
  public hasSynced(): Promise<boolean> {
    console.log("hasSynced is invoked");
  ...
  private performBackup(): Promise<void> {
    console.log("performBackup is invoked");
    return AsyncStorage.getItem(DROPBOX.ACCESS_TOKEN_STORAGE_KEY)
      .then((accessToken) => {
        if (accessToken === null) {
  ...
  private getDatabaseFileMetadataFromDropbox(dropboxFilePath: string, dropboxAccessToken: string): Promise<Response> {
    console.log("getDatabaseFileMetadataFromDropbox is invoked");
  ...
  private getDatabaseName(): string {
    console.log("getDatabaseName is invoked");
    return DATABASE.FILE_NAME;
  }

  private getDatabaseBackupName(): string {
    console.log("getDatabaseBackupName is invoked");
    return DATABASE.BACKUP_FILE_NAME;
  }

  private getDropboxFolder(): string {
    console.log("getDropboxFolder is invoked");
    return "/";
  }

  private getLocalDBFilePath(): string {
    console.log("getLocalDBFilePath is invoked");
    return `/data/user/0/com.rnsqlitedemo/databases/${this.getDatabaseName()}`;
     // return RNFS.LibraryDirectoryPath + "/LocalDatabase/" + this.getDatabaseName();
  }

  private getLocalDBBackupFilePath(): string {
    console.log("getLocalDBBackupFilePath is invoked");
    return `/data/user/0/com.rnsqlitedemo/databases/${this.getDatabaseBackupName()}`;
  }
}
